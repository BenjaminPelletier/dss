...
def print_survivability(node_owners, replicas, replica_per_uss):
  ...
  node_owners = np.array(node_owners)
  ...
  for uss_config in itertools.product((True, False), repeat=n_usss):
    uss_configs.append(np.array(uss_config))
  ...
  for node_config in itertools.product((True, False), repeat=n_nodes):
    node_configs.append(np.array(node_config))
  ...
  for active_nodes_config in itertools.combinations(range(len(node_owners)), replicas):
    ...
    node_active = np.array(tuple(n in active_nodes for n in range(n_nodes)))
  ...
  print_every = datetime.timedelta(seconds=30)
  n_configs_processed = 0

  for uss_up in uss_configs:
    n_usss_down = np.sum(np.logical_not(uss_up))
    for node_active in active_nodes_configs:
      for node_up in node_configs:
        n_other_nodes_down = np.sum(np.logical_and(uss_up[node_owners], np.logical_not(node_up)))
        counts[n_other_nodes_down, n_usss_down] += 1
        up_replicas = np.sum(np.logical_and(np.logical_and(uss_up[node_owners], node_up), node_active))
      ...
      if datetime.datetime.utcnow() > last_print + print_every:
        ...
        last_print = datetime.datetime.utcnow()
        print('{:2f}% complete, {:1f} minutes remaining'.format(fraction_complete * 100, (1 - fraction_complete) * ((last_print - start_time).total_seconds() / 10) / fraction_complete))
...
print_survivability((0, 0, 1, 1, 2, 2, 3, 3), 5, False)
print_survivability((0, 0, 1, 1, 2, 2, 3, 3), 5, True)
